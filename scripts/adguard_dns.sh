...
# Redirect UDP && TCP DNS traffic to 94.140.14.14
sudo iptables -t nat -A OUTPUT -p udp --dport 53 -j DNAT --to-destination 94.140.14.14
sudo iptables -t nat -A OUTPUT -p tcp --dport 53 -j DNAT --to-destination 94.140.14.14
...
sudo iptables -t nat -A POSTROUTING -p tcp --dport 53 -d 94.140.14.14 -j MASQUERADE
